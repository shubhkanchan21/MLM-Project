...
// --- MOCK STATE FOR FRONTEND COMPATIBILITY (quick demo endpoints)
function nowISO(){ return new Date().toISOString(); }
function buildInitialUsers(maxLevel = 6){
  const users = [];
  let id = 1;
  for(let lvl=0; lvl<=maxLevel; lvl++){
    const count = lvl===0 ? 1 : Math.pow(2, lvl);
    for(let i=0;i<count;i++){
      const parentId = lvl===0 ? null : Math.floor(id/2) || null;
      users.push({
        userId: id,
        parentId: parentId,
        level: lvl,
        position: lvl===0 ? null : ((id % 2) === 0 ? 'left' : 'right'),
        name: lvl===0 ? 'Root Admin' : `User ${id}`,
        email: `user${id}@example.com`,
        phone: `+1-555-${String(1000+id).slice(-4)}`,
        role: lvl===0 ? 'admin' : 'user',
        active: true,
        paymentStatus: lvl===0 ? 'COMPLETED' : (lvl<=1 ? 'COMPLETED' : 'NOT_COMPLETED'),
        createdAt: nowISO()
      });
      id++;
    }
...
function buildInitialLevels(maxLevel = 6){
  const levels = [];
  for(let l=0;l<=maxLevel;l++) levels.push({ level: l, unlocked: l===0, unlockedAt: l===0 ? nowISO() : null });
  return levels;
}

const MOCK_STATE = { users: buildInitialUsers(6), levels: buildInitialLevels(6) };
...
function readSeededEmployeesFile() {
  const filePath = path.join(__dirname, 'employees.json');
  const raw = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(raw);
}

async function getSeededEmployeesData() {
  try {
    const result = await pool.query(
      `SELECT
        ...
        joining_date,
        level,
        joining_date::timestamp AS record_timing
       FROM core.employees
       ORDER BY id`
    );
    ...
    // DB unavailable or table missing: use fallback file
  }

  const data = readSeededEmployeesFile().map((row) => ({
    ...row,
    record_timing: row.joining_date
  }));
...
// Public seeded employees endpoint (no auth) - placed before auth waterfall
app.get('/seeded/employees', async (req, res) => {
  try {
    const result = await getSeededEmployeesData();
    if (req.query.with_meta === '1') {
      return res.json({
        report_version: new Date().toISOString(),
    ...
    return res.status(500).json({ error: 'no data available', details: e.message });
  }
});

app.get('/seeded/employees/report', async (req, res) => {
  try {
    const result = await getSeededEmployeesData();
    const levelCounts = {};
    ...
    return res.json({
      report_version: new Date().toISOString(),
      source: result.source,
      total_users: result.data.length,
      level_counts: levelCounts,
      users: result.data
    });
  } catch (e) {
    return res.status(500).json({ error: 'report unavailable', details: e.message });
  }
});
...
// --- QUICK MOCK PUBLIC ENDPOINTS FOR THE FRONTEND
// These routes originally returned the in‑memory MOCK_STATE, but we can
// transparently upgrade them to hit the real database when the pool is
// configured.  This keeps the front‑end code unchanged (it still talks to
// `/mock/...`) while allowing the UI to reflect live data.
app.get('/mock/users', async (req, res) => {
  // support simple paging/search for performance
  ...
  if (pool) {
    try {
      ...
      if (search) {
        params.push(`%${search}%`);
        sql += ` WHERE name ILIKE $${params.length} OR email ILIKE $${params.length}`;
      }
      sql += ` ORDER BY id`;
      if (limit) {
      ...
      console.error('error loading /mock/users from db:', err.message);
      // fall through to return mock state
    }
  }
  res.json(MOCK_STATE.users);
});

app.get('/mock/users/:id', async (req, res) => {
  ...
  if (pool) {
    try {
      ...
      console.error(`error loading /mock/users/${id} from db:`, err.message);
      // continue to fallback
    }
  }

  const u = MOCK_STATE.users.find(x => Number(x.userId) === id);
  if (!u) return res.status(404).json({ error: 'not found' });
  res.json(u);
});

app.get('/mock/levels', async (req, res) => {
  if (pool) {
    try {
      // compute existing levels from the users table
      const result = await pool.query('SELECT MAX(level) AS maxlvl FROM core.users');
      const maxLevel = result.rows[0] ? Number(result.rows[0].maxlvl) : 0;
      const levels = [];
      for (let l = 0; l <= maxLevel; l++) {
        levels.push({ level: l, unlocked: true, unlockedAt: nowISO() });
      }
      return res.json(levels);
    } catch (err) {
      console.error('error loading /mock/levels from db:', err.message);
    }
  }
  res.json(MOCK_STATE.levels);
});

// SERVER ENDPOINTS FOR FRONTEND
// a small report endpoint that mirrors the old seeded/employees report but
// pulls from the real users table when the database is available.  Used by
// `employees_view.html` to display current names and counts.
app.get('/users/report', async (req, res) => {
  ...
  if (pool) {
    try {
      ...
      const result = await pool.query(sql, params);
      const rows = result.rows.map(r => ({ ...r, level: Number(r.level) }));
      const levelCounts = {};
      rows.forEach(r => { levelCounts[r.level] = (levelCounts[r.level] || 0) + 1; });
      return res.json({
        report_version: new Date().toISOString(),
        source: 'database',
      ...
      console.error('error building /users/report:', err.message);
      // fall through to fallback
    }
  }

  // fallback to seeded file behaviour
  const result = await getSeededEmployeesData();
  if (req.query.with_meta === '1') {
    return res.json({
      report_version: new Date().toISOString(),
...
// Serve the employees_view frontend from the workspace root
app.get('/employees_view.html', (req, res) => {
  const filePath = path.join(__dirname, '..', 'employees_view.html');
